...
func NewDeleteUserHandler(uc *usecase.DeleteUserUsecase) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := uc.Delete(ctx, req.UserID); err != nil {
			slog.Error("delete user", "error", err, "request_id", reqID, "user_id", req.UserID)
			writeError(w, err)
			return
		}
